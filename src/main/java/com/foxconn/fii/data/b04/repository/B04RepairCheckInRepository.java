...
public interface B04RepairCheckInRepository extends JpaRepository<B04RepairCheckIn, Integer> {

    @Query("SELECT modelName, count(*) as count FROM B04RepairCheckIn WHERE inputTime BETWEEN :startDate AND :endDate GROUP BY modelName")
    List<Object[]> countByModelName(@Param("startDate") Date startDate, @Param("endDate") Date endDate);

    @Query("SELECT modelName, 'SI' as sectionName, count(*) as count FROM B04RepairCheckIn WHERE inputTime BETWEEN :startDate AND :endDate GROUP BY modelName")
    ...
    List<B04RepairCheckIn> findByInputTimeBetweenAndStationNameNotInAndErrorCodeNotInOrderByInputTimeAsc(Date startDate, Date endDate, List<String> stationName, List<String> errorCode);
