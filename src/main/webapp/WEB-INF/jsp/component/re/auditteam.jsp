            ...
            <tfoot>
                <tr>
                    ...
                    <td style="text-align: center">
                        <input type="radio" onchange="allCheckGoodBad(this)" name="radiocheck" value="3" class="rbtGood">Tốt &nbsp;&nbsp;
                        <input type="radio" onchange="allCheckGoodBad(this)" name="radiocheck" value="4" class="rbtBad">Kém
                    </td>
                    <td></td>
                </tr>
            </tfoot>
...
<script>
    ...
    function getDataDailyLeaderConfirm(leaderId, owner, location) {
        ...
        $.ajax({
            ...
            var str = "";
            let position = 0;
            var info = '<div class="lcO">Auditor: ' + result.leaderId +
            ...
            for (i in result.data) {
                ...
                for (j in dataJ) {
                    ...
                    if (dataJ[j].confirm == 2) {
                        str += '<td> ' +
                            '   <div class="radio-group-fii" cardID="' + leaderId + '" data-name="' + dataJ[j].id + '"><input class="form-check-input-1" type="radio" name="' + dataJ[j].id + '" value="3">Tốt' +
                            '   <input class="form-check-input-2" type="radio" name="' + dataJ[j].id + '" value="4" checked>Kém' +
                            '</div></td>' +
                            '<td class="customPadding"><textarea class="nhanXet" cols="20" rows="2">' + content + '</textarea></td>' +
                            '</tr>';
                    } else if (dataJ[j].confirm == 3) {
                        str += '<td> ' +
                            '   <div class="radio-group-fii" cardID="' + leaderId + '" data-name="' + dataJ[j].id + '"><input class="form-check-input-1" type="radio" name="' + dataJ[j].id + '" value="3" checked>Tốt' +
                            '   <input class="form-check-input-2" type="radio" name="' + dataJ[j].id + '" value="4" >Kém' +
                            '</div></td>' +
                            '<td class="customPadding"><textarea class="nhanXet" cols="20" rows="2">' + content + '</textarea></td>' +
                            '</tr>';
                    } else {
